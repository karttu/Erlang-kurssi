...
otaviestivastaan() ->
  receive
    ATMVIESTI -> ATMVIESTI
    after 10000 ->
      pankkiautomaatti_vissiin_vainaa
  end.

%% Esimerkiksi pano({7,markkaa}).
pano(Rahaa) ->
  atmserver ! {self(),pano,Rahaa},
  otaviestivastaan().

%% Esimerkiksi otto(5). Ei spesifioida mitä valuuttaa, saadaan sitä mitä automaatissa on!
otto(Njotain) ->
  atmserver ! {self(),otto,Njotain},
  otaviestivastaan().
