...
%% Todo: real views of what can be seen. (not just immediate neighbours). (Yes, done now.)

%% Other changes:
%%
%%  2013-04-23: Now with timetick sends also Agentscorelist to showboard_loop in showboard.erl
%%              Now send also {vsn,VERSION_STRING} to agents, after the worldview list.
%%
%%
...
%%           [size(Reply_to_agent),size(Reply_to_agent),element(size(Reply_to_agent),Reply_to_agent),Reply_to_agent]),
  ...
  WHOLE_PACKET = 
    [{agentref,AgentRef},
     ...
     {worldview,Worldview},
     lists:keyfind(vsn,1,?MODULE:module_info(attributes)),
...
%% For fairness: answer to agents in randomized order:
find_movable_agents(Agents_ets,Has_been_able_to_move_since_cycle_N) ->
  lists:sort(fun(_EV,_VK) -> random:uniform(2) == 1 end, %% Probably still far away from absolute fairness...
             find_movable_agents_org(Agents_ets,Has_been_able_to_move_since_cycle_N)
            ).
