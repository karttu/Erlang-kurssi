...
%% agent_corvus.erl -- A simple sample agent that will spot the food from the distance
%%                     and then run towards it. To be used with moveserver.erl
%%
%% After editing a proper hostname to serverhostname.hrl and after starting the moveserver
%% in that host (with -sname moveserver ) you can start this for example as: agent_corvus:init(1).
...
%% agent_corvus:send_game_command(whereis(moveserver),make_ref(),login,[]).
%% agent_corvus:send_game_command(whereis(moveserver),make_ref(),makemove,{move,1}).
%% agent_corvus:send_game_command(whereis(moveserver),make_ref(),makemove,{layblockandmove,1}).
  ...
  receive
    WHOLE_PACKET=[
     ...
     | _REST_FOR_FUTURE_FUNCTIONALITY]
      ...
      agent_corvus:agentloop(Gameserverpid,Myref,
                             Uusisuunta,
    ...
    JokuMuuViesti ->
      ...
      agent_corvus:agentloop(Gameserverpid,Myref,Suunta,Leavingblocks)
  end.
...
%% 
%% Esimerkki:
%% agent_corvus:find_smallest_direction_not_occupied_by_anything([{{1,0},{passive,block,[]}},   {{-1,-1},{passive,block,[]}} , {{0,1}, {blockedby,wall}}])
%% gives 1.
%% agent_corvus:find_smallest_direction_not_blocked_by_wall([{{1,0},{passive,block,[]}},   {{-1,-1},{passive,block,[]}} , {{0,1}, {blockedby,wall}}])
%% gives 0.

%% agent_corvus:find_smallest_direction_not_blocked_by_wall([{{1,0},{blockedby,wall}}, {{1,1},{passive,block,[]}},  {{-1,-1},{passive,block,[]}} , {{0,1}, {blockedby,wall}}]).
%% gives 1.
%% 165> agent_corvus:find_smallest_direction_not_occupied_by_anything([{{1,0},{blockedby,wall}}, {{1,1},{passive,block,[]}},  {{-1,-1},{passive,block,[]}} , {{0,1}, {blockedby,wall}}]).
%% gives 3.
