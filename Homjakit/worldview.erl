...
%%
%% Module worldview implements world_viewed_from(Boardtabref,Boardpos)
%% which returns (currently: "approximately") the items visible
%% from absolute Boardpos of Boardtabref, returned with relative (offset)
%% coordinates.
%%
...
%%                     (Thanks to M. Suovula for the tip!)
%%                     world_viewed_from returns still slightly illogical view:
%%                     it sees through some corners, but not through the others.
%%
%%
...
%%                              [AbsPoint,Relpoint,SomeItem,max_sector_for(Relpoint),Sectors]),
           ...
           _ShadowingSectors -> %% Should really be just one...
%%           io:format("SHADOWED: Abs=~w (Relpoint=~w) contains ~w, Its shading sector=~w, shadowed by=~w.~nSectors=~w~n~n",
%%                              [AbsPoint,Relpoint,SomeItem,max_sector_for(Relpoint),_ShadowingSectors,Sectors]),
             collect_visitems(Boardtabref,Pos,nextpoint_in_spiral(Relpoint),
                              Visitems,Sectors,
