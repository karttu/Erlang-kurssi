...
create_window_short(ParentPID) ->
    Frame = wxFrame:new(wx:null(), -1, "Agentit dallaa", []),

    Panel = wxPanel:new(Frame), 

    Board = showboard_wx_board:new(Panel,ParentPID),
...
handle_info({init, Init}, S = #gs{board=Board,frame=F}) ->
    showboard_wx_board:setup_board(Board, Init),
%%  status(F, "Given ~p  Left ~p", [length(Init), showboard_wx_board:left(Board)]),
...
handle_event(#wx{id=?NEW, event=#wxCommand{}},
    ...
    G ! {op,?NEW,S#gs.level},
    showboard_wx_board:setup_board(Board,[]),
    {noreply, S#gs{orig=[]}};
...
handle_event(#wx{id=?CLEAR, event=#wxCommand{}},
	     S = #gs{game=G,board=Board}) ->    
    Vals = showboard_wx_board:clear_board(Board),
    G ! {loaded, Vals},
    {noreply, S};
...
handle_event(Msg,S) ->
    ...
    %%showboard_wx_board:event(Msg, Ids),
    {noreply, S}.
...
set_val(Id, Val, Board, G) ->
    showboard_wx_board:set_butt(Board, Id,Val),
    G ! {validate, Id, Val},
    ok.
